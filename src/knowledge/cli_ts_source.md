---
title: Knowledge Agent CLI Source Code (v3.4)
description: TypeScript ESM implementation of the Knowledge Agent CLI entrypoint
tags: [vega/v3.4, knowledge-agent, cli, typescript, neo4j]
created: 2026-03-04
modified: 2026-03-04
status: production
vault: Lingelpedia
...
# Knowledge Agent CLI Source Code (v3.4)

This is the complete TypeScript source code for `src/knowledge/cli.ts`. Copy the code block below into your IronClaw repository and add the npm scripts from the package.json updates document.
...
- **Module Resolution**: Configured for TypeScript ESM with `moduleResolution: bundler` in tsconfig.json
- **Error Handling**: Non-blocking try/catch pattern — errors logged to stderr, functions return null on failure
- **Neo4j Import**: Uses `import pg from 'pg'` ESM pattern (note: this placeholder will be replaced with neo4j-driver in production)
- **Process Management**: SIGINT/SIGTERM handlers in `watch` command for graceful shutdown
- **Output Streams**: Results to stdout, progress/errors to stderr
- **Exit Codes**: 0 (success), 1 (error), 2 (validation failure — Phase 1 gate not met)

---

**Related Documents:**
- [[package_json_updates]]
- [[cli_integration_guide]]
- Launch Runbook (forthcoming)
